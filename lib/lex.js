...
function unsafeTokenToOpcode(tokens, mode) {
  ...
  return tokens.map((token, offset) => {
    ...
    if (optimized) {
      var _COPCODE_MAP$nowOp;

      // istanbul ignore next
      return {
        opcode: (_COPCODE_MAP$nowOp = COPCODE_MAP[nowOp]) !== null && _COPCODE_MAP$nowOp !== void 0 ? _COPCODE_MAP$nowOp : nowOp,
        offset
      };
    } else {
      return {
...
function prettify(ops, mode) {
  ...
  ops.forEach(now => {
    switch (mode) {
      ...
      case 'S':
        if (last === OPCODES.ishl && now === OPCODES.iadd) {
          // Sharrk
          output.push(OPCODES.ineg, OPCODES.ineg, OPCODES.iadd);
        } else if (last === OPCODES.isht && now === OPCODES.iadd) {
          // ShaArrk
          output.push(OPCODES.ineg, OPCODES.ineg, OPCODES.iadd);
        } else if (now === OPCODES.onew) {
          // Samee
